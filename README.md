...
- `make install` initializes submodules, installs JS and Leiningen dependencies, creates local config files from examples when missing, builds the frontend bundle, and runs the tests.
...
1. Run `make dev` for a quick reminder of the two-terminal workflow.
2. Start the frontend compiler in one terminal:

```bash
make frontend-watch
```

3. Start the backend server in a second terminal:
